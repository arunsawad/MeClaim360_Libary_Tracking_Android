...
import com.claimdi.databehaviortracking.model.*
import com.google.gson.Gson
import com.claimdi.databehaviortracking.model.*
import com.claimdi.databehaviortracking.mqtt.MqttClien
import com.claimdi.databehaviortracking.utils.DateUtility
import com.claimdi.databehaviortracking.utils.DateUtility.Companion.Time24Pattern
import com.claimdi.databehaviortracking.utils.UtillManager
import com.claimdi.databehaviortracking.InfoBehaviorData
import com.claimdi.databehaviortracking.MqttData
...
class DataBehabviorTracking private constructor(var context: Context) :
    DataBehabviorInterface {
                ...
                if (detail != null) {
                    val publisData =
                        MqttData(
                            ...
                            detail.companyName,
                            detail.companyId,
                            UtillManager.mps_to_kmph(
                                UtillManager.doubleIsNotNull(
                            ...
                            UtillManager.stringIsNotNull(
                                lastLocation.value?.location?.accuracy.toString()
                            ),
                            "accId.value!!",
                            "false"
                        )
                ...
                if (deltaTime > 0 && resultLocation.accuracy <= 40) {
                    lastSpeed.value =
                        DataSpeed(
                            time,
                            UtillManager.convertTo2Digit(
                                ((distanceLocation.value!!) / deltaTime)
                            )
                        )
                } else {
                    lastSpeed.value =
